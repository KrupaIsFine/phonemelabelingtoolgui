...
class MyMainWindow(QMainWindow):
    ...
    def connect_region_signals(self, region_item, region_item2, text_item, var, prob, flag):
    ...
    def update_changed_time(self, var, prob, region_values):
        ...
        self.changed_time[index_current] = [float(start_time), float(end_time), var, prob]
    ...
    def plot_data(self, path):
        ...
        for cell in cells:
            ...
            flag = 0
            self.connect_region_signals(region_item, region_item2, text_item, cell.var, cell.prob, flag)
    ...
    def create_region_item(self, start_time, end_time, var, prob):
        ...
        if prob > avg_prob:
            color = QtGui.QColor(0, 255, 0, 25)
            region_item = LinearRegionItem(values=(start_time, end_time, var, prob),orientation=pg.LinearRegionItem.Vertical, pen=pg.mkPen(color='k', width=1.5), movable=False, brush=pg.mkBrush(color), swapMode='block')
            region_item2 = LinearRegionItem(values=(start_time, end_time, var, prob),orientation=pg.LinearRegionItem.Vertical, pen=pg.mkPen(color='k', width=1.5), movable=False, brush=pg.mkColor(0, 0, 0, 0), swapMode='block')
        else:
            color = QtGui.QColor(255, 0, 0, 25)
            region_item = LinearRegionItem(values=(start_time, end_time, var, prob),orientation=pg.LinearRegionItem.Vertical, pen=pg.mkPen(color='k', width=1.5), brush=pg.mkBrush(color), swapMode='block')
            region_item2 = LinearRegionItem(values=(start_time, end_time, var, prob),orientation=pg.LinearRegionItem.Vertical, pen=pg.mkPen(color='k', width=1.5), movable=False, brush=pg.mkColor(0, 0, 0, 0), swapMode='block')            
        ...
        border_color = QtGui.QColor(255, 0, 0, 100)  # Red border
        fill_color = QtGui.QColor(240, 240, 240, 255)  # Green fill with alpha value 100
        text_item = pg.TextItem(text=f"{var}\n{prob}", border=border_color, fill=fill_color)
        self.text_items.append(text_item)
        self.plot_item.addItem(text_item)
        ...
        # region_item.clicked.connect(self.display_clicked_values)
        flag = 1
        self.connect_region_signals(region_item, region_item2, text_item, var, prob, flag)
