...
class LinearRegionItem(pg.LinearRegionItem):
    ...
    def play_audio(self, start, end):
        ...
        start_index = int((start_time / 1e7) * sample_rate_gl)
        end_index = int((end_time / 1e7) * sample_rate_gl)
        if DEBUG:
        ...
        if DEBUG:
            print("Sample Rate:", sample_rate_gl)
        # Start audio playback
        sd.play(audio_segment, samplerate=sample_rate_gl, blocksize=blocksize)
...
class MyMainWindow(QMainWindow):
    ...
    def connect_region_signals(self, region_item, region_item2, text_item, var, prob, region_bounding_rect):
        ...
        region_item.sigRegionChangeFinished.connect(lambda: self.adjust_next_region(region_item, start_time, end_time, var, prob, text_item, region_bounding_rect, flag=0))
    ...
    def adjust_next_region(self, current_region_item, st, et, var, prob, text_item, region_bounding_rect, flag):
        s = st
        e = et
        v = var
        p = prob
    ...
    def plot_data(self, path):
        ...
        for cell in cells:
            ...
            self.connect_region_signals(region_item, region_item2, text_item, cell.var, cell.prob, region_bounding_rect)
