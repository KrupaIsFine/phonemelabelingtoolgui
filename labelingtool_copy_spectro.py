...
from pycutie_5 import Ui_MainWindow
from scipy.signal import spectrogram
from pyqtgraph import ColorMap

# Create a ColorMap instance with the 'viridis' colormap
cmap = ColorMap([0.0, 0.5, 1.0], [(0, 0, 0), (0, 255, 255), (0, 0, 0)])
...
class MyMainWindow(QMainWindow):
    def __init__(self):
        ...
        self.plot_widget2 = pg.PlotWidget(background='w') 
        self.plot_widget2.getPlotItem().setContentsMargins(0, 0, 0, 0)
        self.spectrogram_img = ImageItem()
        self.plot_widget2.addItem(self.spectrogram_img)
        # scene2 = QGraphicsScene()
        scene.addWidget(self.plot_widget2)
        self.ui.graphicsView.setScene(scene)
        ...
        # Enable mouse interactions for zooming
        self.plot_widget2.setMouseEnabled(x=True, y=False)
    ...
    def plot_data(self, path):
        ...
        # self.plot_widget2.getAxis("bottom").setVisible(True)
        # self.plot_widget2.getAxis("left").setVisible(True)
        self.plot_widget.setXRange(cells[0].st, cells[-1].et)
        # self.plot_widget2.setXRange(cells[0].st, cells[-1].et)
    ...
    def plot_graph(self, path):
        ...
        img = librosa.display.specshow(spec, sr=sample_rate, ax=ax)
        ...
        self.plot_widget2.addItem(pixmap_item)
        self.plot_widget2.setSceneRect(pixmap_item)
    ...
    #     y_axis2 = self.plot_widget2.getAxis("left")
    #     # Get the current range of the y-axis
    #     y_range = y_axis.range
    #     y_range2 = y_axis2.range
    #     return y_range, y_range2

    def resizeEvent(self, event):
        # Resize the plot widget along with the central widget
        new_size = event.size()
        self.plot_widget.setGeometry(0, 0, new_size.width(), 410)
        self.plot_widget2.setGeometry(0, 0, new_size.width(), 410)
        event.accept()
