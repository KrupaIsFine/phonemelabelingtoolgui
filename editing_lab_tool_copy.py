...
from pycutie_3_copy import Ui_MainWindow # Editing version of UI File
...
DEBUG = False
dct_obj = {}
trans_color = QtGui.QColor(0, 0, 0, 0)
...
class LinearRegionItem(pg.LinearRegionItem):
    ...
    def highlight_region(self):
        ...
        # Set the brush color
        print("new brush color: ", new_brush_color)
        trans_color = QtGui.QColor(0, 0, 255, 25)
...
class MyMainWindow(QMainWindow):
    def __init__(self):
        ...
        self.ui.pushButton_13.clicked.connect(self.add_text_panel)
        ...
        self.plot_item0 = pg.PlotItem()
        self.plot_item0.setFixedHeight(100)  # Set fixed width and height
        ...
        p0 = self.layout_widget.addItem(self.plot_item0)
        ...
        # if p1 is not None and p2 is not None and p0 is not None:
        #     if DEBUG:
        ...
        self.plot_item0.setMouseEnabled(x=True, y=False)
    ...
    # def plot_iitems(self, plot_item):
    #     plot_item.setFixedHeight(100)
    #     self.layout_widget.setStyleSheet("border: 1px solid black;")
    #     self.layout_widget.addItem(plot_item)
    #     self.layout_widget.ci.layout.setRowSpacing(0, 0)
    #     self.layout_widget.setBackground('w')  # Set background color to white
    #     self.layout_widget.nextRow()
    ...
    def add_text_panel(self):
        global n
        plot_item = plot_item + n
        print("plot_item: ", plot_item)
        self.plot_item = pg.PlotItem()
        # plot_iitems(self.plot_item)





    # Define the slot method to handle resizing
    def handlePlotItemResized(self, text_item):
        # Calculate the new size of the text item based on the zoom level or size of the plot item
        # print("plot item viewbox: ", self.plot_item0.getViewBox().viewRec
        # Set contents margins to zero
        # Set border around the layout widget
        self.layout_widget.setStyleSheet("border: 1px solid black;")
        p0 = self.layout_widget.addItem(self.plot_item0)
        ...
        self.layout_widget.setBackground('w')  # Set background color to white
        self.layout_widget.nextRow()
        p1 = self.layout_widget.addItem(self.plot_item1)
        new_size = self.plot_item0.getViewBox().viewRect().size()

        # Set the new size for the text item
        # text_item.setFont(QtGui.QFont("Arial", 15))
        text_item.setFont(QtGui.QFont("Arial", int(new_size.height() / 10)))  # Adjust font size based on the plot item size
    ...
    #         self.rectChanged.emit(self.rubberBand.geometry())
    #     super(MyMainWindow, self).mouseMoveEvent(event)
    ...
    #                     item.cambi(True)               
    #     super(MyMainWindow, self).mouseReleaseEvent(event)


    def zoom_in(self):
        # Increase zoom factor
        self.plot_item0.getViewBox().scaleBy((0.5, 0.5))
        # print("Zoom In:", self.zoom_factor)

    def zoom_out(self):
        # Decrease zoom factor
        self.plot_item0.getViewBox().scaleBy((2, 2))
    ...
    def adjustLayoutWidgetSize(self, rect):
        ...
        self.plot_item0.setGeometry(rect.toRect())
    ...
    def connect_region_signals(self, region_item, region_item1, region_item2, text_item, var, prob):
        ...
        region_item.sigRegionChanged.connect(lambda: self.sync_region_items(region_item, region_item1, region_item2))
        if DEBUG:
        ...
        region_item.clicked.connect(lambda: self.sync_region_items(region_item, region_item1, region_item2))
    ...
    def reset(self):
        ...
        for region_item in self.region_items:
            self.plot_item0.removeItem(region_item)
        self.region_items = []
    ...
    def handle_label_update(self, region_item, text_item, var, prob):
        ...
        # center_x = region_rect.x() + (region_rect.width() - text_width) / 2
        # center_y = region_rect.y() + (region_rect.height() - text_height) / 2

        # # Set the position of the text item to the calculated center position
        # text_item.setPos(center_x, center_y)
        text_item.setPos((label_x + label_y) / 2, 0.5)
        # text_item.setPos(label_x, 0.5)

    def sync_region_items(self, source_region_item, target_region_item, target2_region_item):
        ...
        print("source item color: ", source_region_item.brush.color())
        br_name = source_region_item.brush.color().name()
        print("br_name: ", br_name)
        if br_name != '#ff0000' and br_name != '#00ff00':
            target_region_item.setBrush(source_region_item.brush.color())
            print("brush1 changed")
            target2_region_item.setBrush(source_region_item.brush.color())
            print("brush2 changed")
        else:
            target_region_item.setBrush(pg.mkColor(0, 0, 0, 0))
            print("brush1 changed")
            target2_region_item.setBrush(pg.mkColor(0, 0, 0, 0))
            print("brush2 changed")              
        
        target_region_item.setRegion([st, et])
  
        
        target2_region_item.setRegion([st, et])
        if DEBUG:
            print("after syncing")
    ...
    def plot_data(self, path):
        ...
        for cell in cells:
            if cell.prob > avg_prob:
                ...
                region_item1 = LinearRegionItem(values=(cell.st, cell.et,cell.var, cell.prob),orientation=pg.LinearRegionItem.Vertical, pen=pg.mkPen(color=trans_color, width=1.5), movable=False, brush=pg.mkColor(0, 0, 0, 0), swapMode='block')
                region_item2 = LinearRegionItem(values=(cell.st, cell.et,cell.var, cell.prob),orientation=pg.LinearRegionItem.Vertical, pen=pg.mkPen(color=trans_color, width=1.5), movable=False, brush=pg.mkColor(0, 0, 0, 0), swapMode='block')
            else:
            ...
            index += 1
            self.plot_item0.addItem(region_item)
            self.plot_item.addItem(region_item1)
            ...
            self.plot_item0.addItem(text_item)
            ...
            # print("viewbox specs: ", self.plot_item0.getViewBox())
            # self.plot_item0.getViewBox().sigResized.connect(lambda: self.handlePlotItemResized(text_item))
            self.connect_region_signals(region_item, region_item1, region_item2, text_item, cell.var, cell.prob)
            self.handle_label_update(region_item, text_item, cell.var, cell.prob)
            # self.plot_item0.getViewBox().sigResized.connect(lambda: self.handlePlotItemResized(text_item))

        self.plot_item0.setYRange(0, 1)
    ...
    def plot_graph(self, path):
        ...
        self.plot_item0.getAxis('left').setWidth(int(75))
        self.plot_item0.getAxis("left").setTicks([]) 
        self.plot_item0.getAxis("bottom").setTicks([]) 
        self.plot_item2.setXRange(0, audio_duration)
        self.plot_item2.setYRange(0, (sample_rate / 2) - 12000)
        self.plot_item0.setXRange(0, audio_duration)
        # self.plot_item2.setYLink(self.plot_item)
        self.plot_item.setXLink(self.plot_item0)
        self.plot_item2.setXLink(self.plot_item0)

    def resizeEvent(self, event):
        # Resize the plot widget along with the central widget
        new_size = event.size()
        self.plot_item0.setGeometry(0, 0, new_size.width(), 400)
