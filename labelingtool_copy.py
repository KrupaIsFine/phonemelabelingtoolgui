...
class MyMainWindow(QMainWindow):
    ...
    def process_input_values(self):
        ...
        new_cell = start_time, end_time, variable_text, probability
        create_cells(new_cell)  # Assuming create_cells accepts a Phonemes object as input
        self.create_region_item(start_time, end_time, variable_text, probability)
    ...
    def plot_data(self, path):
        ...
        self.plot_widget.setLabel("bottom", text="Time")
    ...
    def create_region_item(self, start_time, end_time, variable, probability):
        # Create the new region item
        region_item = LinearRegionItem(values=(start_time, end_time, variable, probability),
        ...
        self.region_lst.insert(insert_index, [insert_index, [start_time, end_time, variable, probability]])
        # Add the region item to the plot widget
        self.plot_widget.addItem(region_item)
        print(f"New region item created for Variable: {variable}")
        self.plot_widget.addItem(region_item)
        text_item = pg.TextItem(text=f"{variable}\n(Prob: {probability})", anchor=(0, 0))
        self.text_items.append(text_item)
        self.plot_widget.addItem(text_item)
        print("Text items of new region: ", self.text_items)
        # Connect the clicked signal of the region item to a slot
        region_item.clicked.connect(self.display_clicked_values)
        self.connect_region_signals(region_item, text_item, variable, probability)
        self.handle_label_update(region_item, text_item, variable, probability)
        print(f"New region item created for Variable: {variable}")
